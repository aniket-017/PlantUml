  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Edit Test Cases</h1>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Test Cases ({state.testCases.length})</h2>
            <div className="flex justify-center sm:justify-end">
              <button
                onClick={handleGenerateDiagram}
                disabled={state.loading || state.testCases.length === 0}
                className="flex items-center px-4 sm:px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors text-sm sm:text-base"
              >
                {state.loading ? (
        ...
        <div className="divide-y divide-gray-200">
          {state.testCases.length === 0 ? (
            ...
            state.testCases.map((testCase) => (
              <div key={testCase.id} className="p-4 sm:p-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 space-y-2 sm:space-y-0">
                      <div className="flex-1">
                        <div className="flex items-center mb-2">
                          ...
                          <h3 className="text-base sm:text-lg font-medium text-gray-800">{testCase.title}</h3>
                        </div>
                        <p className="text-xs sm:text-sm text-gray-500">ID: {testCase.id}</p>
                      </div>
                      <button
                        onClick={() => handleEdit(testCase)}
                        className="flex items-center justify-center px-3 py-2 text-blue-600 hover:text-blue-800 bg-blue-50 rounded-md transition-colors text-sm"
                      >
                      ...
                      <div className="mb-4">
                        ...
                        {testCase.steps.map((step, index) => {
                          ...
                          return (
                            <div
                              key={index}
                              className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 p-4 rounded-lg mb-3"
                            >
